...
class Reports extends AdminController
{
    ...
    {   
        ...
        $user_login_id = '';
        $user_login_id = $this->session->userdata("staff_user_id");

        $this->db->select("admin");
        $this->db->from(db_prefix()."staff as staff");
        $this->db->where("staffid", $user_login_id);
        $staff_query = $this->db->get();
        $staffResult = $staff_query->row_array();
        $is_admin = 0;
        if(!empty($staffResult))
        {
            $is_admin = $staffResult['admin'];
        }
       ...
       $this->db->select("tsa.id as id, 
        ...
        IFNULL(rm.lastname, 'NA') AS reporting_manager_lastname,
        IFNULL(p.name, 'NA') AS project_name,
        IFNULL(c.company, 'NA') AS client_name,
        tsa.status,
        tsa.comment, 
        tsa.teamlead_status,
        tsa.teamlead_comment,
        tsa.reporting_manager_id,
        tsa.from_date as start_from,
        tsa.to_date as start_to,
        tsa.created_at as timesheet_added,
        tsa.pdf_url,
        tsa.excel_url
        ");
        $this->db->from(db_prefix().'time_sheet_approval as tsa');
        $this->db->join(db_prefix()."staff as staff","staff.staffid = tsa.staff_id","inner");
        ...
        if($user_login_id !="" && $is_admin == '0')
        {
            $this->db->group_start();
            $this->db->where("tsa.staff_id", $user_login_id);
            $this->db->or_where("tsa.reporting_manager_id", $user_login_id);
            $this->db->group_end();
        }
        ...
        // $this->db->group_by("tsa.id, staff.firstname, rm.firstname, c.company, tsa.status");
        ...
        if(!empty($rResult))
        {
            ...
            foreach ($rResult as $aRow) {
                ...
                $this->db->select("cv.value as is_timesheet_approval");
                $this->db->from(db_prefix()."customfieldsvalues as cv");
                $this->db->join(db_prefix()."customfields as cf","cv.fieldid = cf.id AND cv.fieldto = 'staff' and cf.slug='staff_is_timesheet_approval'","inner");
                ...
                $row[] = $i;
                $staff_name = '';
                if(isset($aRow['staff_name']) && $aRow['staff_name'] != "")
                {
                    $staff_name .=$aRow['staff_name'];
                }
                if(isset($aRow['staff_lastname']) && $aRow['staff_lastname'] != "")
                {
                    $staff_name .=' '.$aRow['staff_lastname'];
                }

                $row[] = date("d/m/Y H:i:s", strtotime($aRow['timesheet_added']));
                $row[] = $staff_name;
                $row[] = date("d/m/Y", strtotime($aRow['start_from']));
                $row[] = date("d/m/Y", strtotime($aRow['start_to']));

                // reporting_manager_lastname
                $rm_name = '';
                if(isset($aRow['reporting_manager_name']) && $aRow['reporting_manager_name'] != "")
                {
                    $rm_name .= $aRow['reporting_manager_name'];
                }

                if(isset($aRow['reporting_manager_lastname']) && $aRow['reporting_manager_lastname'] != "")
                {
                    $rm_name .= ' '.$aRow['reporting_manager_lastname'];
                }
                ...
                if($aRow['teamlead_status'] == "0")
                {
                    $teamlead_status .= '<span class="bg-warning spanTextButton">Pending</span>';
                    
                    if($is_admin == '1' || $aRow['reporting_manager_id'] ==  $user_login_id)
                    {
                        $teamlead_action .= '<button type="button" data-toggle="modal" data-target="#myModalApprove" class="btn btn-success btn-xs open-modal" data-rowid="'.$aRow['id'].'" data-approval_level ="1" data-type="approve">'._l('approve').'</button>';
                        $teamlead_action .= '  ';
                        $teamlead_action .= '<button type="button" data-toggle="modal" data-target="#myModalReject" class="btn btn-danger btn-xs open-modal" data-rowid="'.$aRow['id'].'" data-approval_level ="1" data-type="reject">'._l('reject').'</button>';
                    }
                    
                }
                elseif($aRow['teamlead_status'] == "1")
                {
                    $teamlead_status .= '<span class="bg-success spanTextButton">'._l('approved').'</span>';
                }
                elseif($aRow['teamlead_status'] == "2")
                {
                    $teamlead_status .= '<span class="bg-danger spanTextButton">'._l('rejected').'</span>';
                }
               
                $row[] = $teamlead_status;
                $row[] = $aRow['teamlead_comment'];

                $status = '';
                $action = '';

                if($aRow['status'] == "0" && $aRow['teamlead_status']== '1')
                {
                    $status .= '<span class="bg-warning spanTextButton">Pending</span>';
                    if(($is_admin == '1' || $aRow['is_timesheet_approval'] != ""))
                    {
                ...
                if(isset($aRow['pdf_url']) && $aRow['pdf_url'] != "")
                {
                    $file_downlaod .= '<a class = "btn btn-danger" href="'.$aRow['pdf_url'].'" target= "_blank" title="PDF Downlaod"><i class="far fa-file-pdf"></i></a>';
                    $file_downlaod .= ' <br><br>';
                }

                if(isset($aRow['excel_url']) && $aRow['excel_url'] != "")
                {
                    $file_downlaod .= '<a class = "btn btn-success" href="'.$aRow['excel_url'].'" target= "_blank" title="Excel Downlaod"><i class="far fa-file-excel"></i></a>';
                }
    ...
    public function teamlead_timesheet_approve()
    {
        $id = $_POST['id'];
        if($id > 0)
        {
    ...
    public function teamlead_timesheet_reject()
    {
        $id = $_POST['id'];
        if($id > 0)
        {
