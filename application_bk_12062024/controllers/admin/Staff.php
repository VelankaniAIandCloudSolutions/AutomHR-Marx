    ...
    {   
        ...
        if(!empty($logged_time['timesheets']) && sizeof($logged_time['timesheets']) > 0 )
        {
            ...
            if(!empty($time_sheet_exist_results))
            {
                ...
                $subject = 'Timesheet Approval Request from '.ucwords(get_staff_full_name($id)).'  for the month of '.date('M/Y', strtotime($period_from));
                
                $message = "Hi ".ucwords($approval_name).",<br>

                            ".ucwords(get_staff_full_name($id))." has submitted a timesheet for your approval. Please review and respond to this request by following the link below:<br>

                            <a href='".base_url('admin/reports/timesheet_approval_list')."' target='_blank'> Click Here </a><br>

                            Your prompt attention to this matter is greatly appreciated.<br>
       ...
       if(!empty($post_data))
       {
        ...
        if(!empty($timesheet_task_records))
        {
            ...
            if(isset($post_data['day_display']) && $post_data['day_display'] == '1')
            {
             ...
             // print_r($tmp_timesheet_task_records); die;
